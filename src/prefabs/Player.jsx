import { Suspense, useEffect, useRef, useState } from "react";
import { useSphere } from "@react-three/cannon";
import { useThree, useFrame } from "@react-three/fiber";
import { Vector3 } from "three";
import Weapon from "./Weapon";
import useVariable from "../hooks/useVariable"
import useKeyboardInput from "../hooks/useKeyboardInput"
import useMouseInput from "../hooks/useMouseInput"

/** Player movement constants */
const speed = 200;
const attackSpeed = 30;
const attackCoolDown = 300;
const jumpSpeed = 4;
const jumpCoolDown = 400;
const rotation = new Vector3()

const Player = (props) => {
  const weapon = useRef();

  const [action, setAction] = useState("Idle")

  /** Player collider */
  const [ref, api] = useSphere(() => ({
    mass: 100,
    fixedRotation: true,
    position: [0, 5, 0],
    args: [1],
    material: {
      friction: 0
    },
    ...props
  }));
  
  /** Input hooks */
  const pressed = useKeyboardInput(["w", "a", "s", "d", " "]);
  const pressedMouse = useMouseInput();

  /** Converts the input state to ref so they can be used inside useFrame */
  const input = useVariable(pressed);
  const mouseInput = useVariable(pressedMouse);

  /** Player movement constants */
  const { camera } = useThree();

  /** Player state */
  const state = useRef({
    timeToAttack: 0,
    timeTojump: 0,
    vel: [0, 0, 0],
    jumping: false,
  });

  useEffect(() => {
    api.velocity.subscribe((v) => (state.current.vel = v));
  }, [api]);


  /** Handles movement */
  const setMovement = (delta) => {
    const { w, s, a, d } = input.current;
    const space = input.current[" "];

    let velocity = new Vector3(0, 0, 0);
    ref.current.getWorldPosition(camera.position);
    
    let forward = new Vector3();
    forward.setFromMatrixColumn(camera.matrix, 0);
    forward.crossVectors(camera.up, forward);

    let right = new Vector3();
    right.setFromMatrixColumn(camera.matrix, 0);

    let [horizontal, vertical] = [0, 0];

    if (w) {
      vertical += 1;
    }
    if (s) {
      vertical -= 1;
    }
    if (d) {
      horizontal += 1;
    }
    if (a) {
      horizontal -= 1;
    }

    if (horizontal !== 0 && vertical !== 0) {
      velocity
        .add(forward.clone().multiplyScalar(speed * vertical))
        .add(right.clone().multiplyScalar(speed * horizontal));
      velocity.clampLength(-speed, speed);
    } else if (horizontal !== 0) {
      velocity.add(right.clone().multiplyScalar(speed * horizontal));
    } else if (vertical !== 0) {
      velocity.add(forward.clone().multiplyScalar(speed * vertical));
    }

    /** Updates player velocity */
    api.velocity.set(
      velocity.x * delta,
      state.current.vel[1],
      velocity.z * delta
    );

    
    /** Handles jumping */
    if (state.current.jumping && state.current.vel[1] < 0) {
      /** Ground check */
      if(weapon.current.position.y <= 1)
        state.current.jumping = false;
    }

    if (space && !state.current.jumping) {
      const now = Date.now();
      if (now > state.current.timeTojump) {
        state.current.timeTojump = now + jumpCoolDown;
        state.current.jumping = true;
        api.velocity.set(state.current.vel[0], jumpSpeed, state.current.vel[2]);
      }
    }
    
    /** Updates weapon position */
    weapon.current.rotation.copy(camera.rotation);
    weapon.current.position.copy(camera.position).add(camera.getWorldDirection(rotation).multiplyScalar(1));

    
    /** Handles Attack */
    if (mouseInput.current.left) {
      const now = Date.now();
      if (now >= state.current.timeToAttack) {
        state.current.timeToAttack = now + attackCoolDown;
      }
    }

    /** Update Animation State */
    if(horizontal !== 0 || vertical !== 0) setAction("Walk")
    else if(space) setAction("Run")
    else if(mouseInput.current.left) setAction("Attack_right")
    else setAction("Idle") 
  }

  useFrame((_,delta) => {
    setMovement(delta)
  });

  return (
    <>
      <mesh ref={ref} />
      <group ref={weapon} >
          <Weapon action={action} />
      </group>
    </>
  );
};

export default Player;
